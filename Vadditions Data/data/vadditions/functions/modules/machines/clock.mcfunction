#@s - console
#called by #vadditions:modules/machines/clock
...
execute as @e[type=minecraft:item,nbt={Item:{Count:1b,tag:{isMachine:1b,VADS_Machine:"vadditions:bonsai_pot"}},OnGround:1b},tag=!VADS_NoSummon,scores={VADS_BnsPotCnt=1}] at @s if block ~ ~ ~ #minecraft:air align xz run function vadditions:modules/machines/bonsai_pots/create/empty
#sand extractors
    #creation check
...
execute as @e[type=minecraft:item,nbt={Item:{Count:1b,tag:{isMachine:1b,VADS_Machine:"vadditions:sand_extractor"}},OnGround:1b},tag=!VADS_NoSummon,scores={VADS_ExtrctrCnt=1}] at @s if block ~ ~ ~ #minecraft:air align xz run function vadditions:modules/machines/sand_extractors/create
    #visual check

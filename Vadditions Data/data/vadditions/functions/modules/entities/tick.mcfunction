...
execute as @e[type=minecraft:armor_stand,tag=!VADS_Entity] at @s run function vadditions:modules/entities/processes/observer_notification_stands
...
#damaging snowballs
execute as @e[type=minecraft:snowball,tag=!VADS_DamagedSnowball] at @s run function vadditions:modules/entities/processes/damaging_snowballs
#white castle master
    #freeze effect
...
execute at @e[scores={VADS_FrzUseTimer=199},tag=VADS_WhiteCastleMaster] if entity @p[distance=..10,gamemode=!spectator,gamemode=!creative,nbt=!{ActiveEffects:[{Id:14b}]}] run function vadditions:modules/visual/run/freeze
execute at @e[scores={VADS_FrzUseTimer=180..198},tag=VADS_WhiteCastleMaster] if entity @p[distance=..10,gamemode=!spectator,gamemode=!creative,nbt=!{ActiveEffects:[{Id:14b}]}] run function vadditions:modules/visual/charge_up
    #sounds
...
execute as @e[tag=VADS_ConfusedEffect] if score @s VADS_CnfsedTime matches 100.. run function vadditions:modules/entities/effects/remove/confused
